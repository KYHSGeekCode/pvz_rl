...
class Zombie(Entity):
    ...
    def __init__(
        self, lane, pos=config.LANE_LENGTH - 1
    ):  # Zombie starts at the very right
        """
        ...
        self._cell_length = (
            self.WALKING_SPEED * config.FPS
        )  # How many frames to pass a cell
        self._offset = (
            self._cell_length - 1
        )  # Offset used to model the displacement of the zombie on its cell
