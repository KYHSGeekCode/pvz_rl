...
class Scene:
    def __init__(self, plant_deck, zombie_spawner):
        ...
        self.plant_cooldowns = {
            plant: 0 for plant in plant_deck
        }  # All plants are available at the beginning of the level

        self.grid = Grid()

        self._zombie_spawner = (
            zombie_spawner  # Used to spawn the zombies we want for the level
        )
        self._timer = (
            config.NATURAL_SUN_PRODUCTION_COOLDOWN * config.FPS - 1
        )  # Natural production of sun
        ...
        self._render_info = [
            {
                ...
                "sun": self.sun,
                "score": self.score,
                "cooldowns": {name: 0 for name in self.plant_cooldowns},
                "time": 0,
            }
        ]
    ...
    def _render_get_info(self):
        info = {
            ...
            "score": self.score,
            "cooldowns": {
                name: int(self.plant_cooldowns[name] / config.FPS) + 1
                for name in self.plant_cooldowns
            },
            "time": int(self._chrono / config.FPS),
        }
        for zombie in self.zombies:
            info["zombies"][zombie.lane].append(
                (zombie.__class__.__name__, zombie.pos, zombie.get_offset())
            )
        for projectile in self.projectiles:
            if projectile._render():
                info["projectiles"][projectile.lane].append(
                    (projectile.__class__.__name__, projectile._pos, projectile._offset)
                )
        for plant in self.plants:
    ...
    def move_available(self):  # Return true if a player can make a move
        if not self.grid.is_full():
            for plant_name in self.plant_deck:
                if (self.plant_cooldowns[plant_name] <= 0) and (
                    self.plant_deck[plant_name].COST <= self.sun
                ):
                    return True
        return False
    ...
    def __str__(self):
        grid = np.full(
            (config.N_LANES, config.LANE_LENGTH, 3), ["______", "_", "_"], dtype=object
        )
        for plant in self.plants:
            grid[plant.lane, plant.pos][0] = (
                plant.__class__.__name__[0] + ":" + str(plant.hp).zfill(4)
            )
