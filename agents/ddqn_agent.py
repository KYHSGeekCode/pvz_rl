...
def sum_onehot(grid):
    return torch.cat(
        [torch.sum(grid == (i + 1), axis=-1).unsqueeze(-1) for i in range(4)], axis=-1
    )
...
class QNetwork(nn.Module):
    ...
    def get_qvals(self, state, use_zombienet=True):
        if type(state) is tuple:
            ...
            zombie_grid = state_t[:, self._grid_size : (2 * self._grid_size)].reshape(
                -1, config.LANE_LENGTH
            )
            plant_grid = state_t[:, : self._grid_size]
            if self.use_zombienet:
                zombie_grid = self.zombienet(zombie_grid).view(
                    -1, self.zombienet_output_size * config.N_LANES
                )
            else:
            ...
            state_t = torch.cat(
                [plant_grid, zombie_grid, state_t[:, 2 * self._grid_size :]], axis=1
            )
        else:
            state_t = torch.FloatTensor(state).to(device=self.device)
            zombie_grid = state_t[self._grid_size : (2 * self._grid_size)].reshape(
            ...
            if self.use_gridnet:
                plant_grid = self.gridnet(plant_grid)
            state_t = torch.cat(
                [plant_grid, zombie_grid, state_t[2 * self._grid_size :]]
            )
        ...
        if done:
            if mode != "explore":  # We document the end of the play
                self.training_iterations.append(
                    min(config.MAX_FRAMES, self.env._scene._chrono)
                )
