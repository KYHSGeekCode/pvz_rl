...
class ReinforceAgentV2:
    ...
    def decide_action(self, observation):
        ...
        action = np.random.choice(self.possible_actions, 1, p=action_prob.data.numpy())[
            0
        ]
        return action
    ...
    def iterate_minibatches(
        self, observation, actions, rewards, batchsize, shuffle=False
    ):
    ...
    def update(self, observation, actions, rewards):
        ...
        # L = log π(a | s ; θ)*A
        loss = 0
        for observation_batch, action_batch, reward_batch in self.iterate_minibatches(
            observation, actions, rewards, batchsize=100, shuffle=True
        ):
            # import pdb; pdb.set_trace()
            mask_batch = (
                torch.Tensor([self._get_mask(s) for s in observation_batch])
    ...
    def _get_mask(self, observation):
        empty_cells = np.nonzero(
            (observation[: self._grid_size] == 0).reshape(
...
if __name__ == "__main__":
    ...
    for episode_idx in range(n_iter):
        ...
        if episode_idx % n_record == n_record - 1:
            ...
            print(
                "---Episode {}, mean score {}".format(episode_idx, sum_score / n_record)
            )
